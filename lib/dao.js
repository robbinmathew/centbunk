/**
 * Created by pmathew on 12/13/15.
 */
var Sequelize = require('sequelize');


var sequelize = new Sequelize('PBMS_JUN_2011', 'root', 'test', {
    host: 'localhost',
    port:3306,
    dialect: 'mysql',
...
// All the model definitions here
var Party = sequelize.define('Party', {
    partyId: {
        type: Sequelize.INTEGER,
        primaryKey: true,
        field: 'PK_PARTY_ID' // Will result in an attribute that is firstName when user facing but first_name in the database
    },
    partyName: {
    ...
    tableName: 'pbms_party_table'
});

function DatabaseModel() {
    this.getParties = function(type, includeInactive, callback) {
        var idLowerLimit;
        var idHigherLimit;
        var status = "ACTIVE%";
        if( type === 'employee' ) {
    ...
    //Private
    this.getAllInternal = function(model, whereClause, orderClause, callback ) {
        model.findAll({
            where: whereClause,
            order: orderClause
            ...
            console.error('DAO:Error while reading %s, err', model, err);
        });
    }
}
...
/*sequelize.query('SELECT  `PK_PARTY_ID` AS `partyId`, `PARTY_NAME` AS `partyName`, `PARTY_DETAIL` AS `partyDetail`, `PARTY_PHONE` AS `partyPhone`, `PARTY_STATUS` AS `partyStatus` FROM pbms_party_table', { model: Party }).then(function(result){//
    console.log(" Party name:%j", result[0]);
    // Each record will now be a instance of Project
}).catch(function(err) {
    console.log(err);
});

var parties1 = Party.findOne({
    where: { partyId: 1 }
}).then(function(john) {
    console.log(" john name:%j", john);
}).catch(function(err) {
    console.log(err);
});
*/
...
//console.log( "test %j", new DatabaseModel().getParties('employee'));
...
/*
 ...
 {
 return getByQuery( ProdRateChange.class, "from ProdRateChange where date <=? order by date,prodId, slNo", date );
 }
