    ...
    public void run() {
        try {
            ...
            try {
                ...
                if (lastScanStatus.shouldRun(indianDate, ScrapedDetail.PRICE_CHANGE_TYPE)) { //Avoid re-run if already read for today
                    LastScanStatus.ScanStatus scanStatus =  lastScanStatus.getLastScanStatusMap(ScrapedDetail.PRICE_CHANGE_TYPE);
                    ...
                    if (prices.get("DATE") != null && prices.get("DATE").equals(DateUtil.getIndianDateString(currTime))) {
                        ScrapedDetail newPriceValues = new ScrapedDetail();
                        newPriceValues.setType(ScrapedDetail.PRICE_CHANGE_TYPE);
                ...
                if (lastScanStatus.shouldRun(indianDate, ScrapedDetail.HPCL_TRX)) { //Avoid re-run if already read for today
                    LastScanStatus.ScanStatus scanStatus =  lastScanStatus.getLastScanStatusMap(ScrapedDetail.HPCL_TRX);
